...
export function useProducts() {
  ...
  const updateProduct = async (id: string, record: Partial<Product>) => {
    const { error } = await (supabase as any).from("products").update(record).eq("id", id);
    if (error) {
      toast({ title: "Erreur", description: error.message, variant: "destructive" });
      return false;
    }
...
export function useProductAttributes() {
  ...
  const addAttributeValue = async (attribute_id: string, value: string, color_hex?: string) => {
    const { error } = await (supabase as any)
      .from("product_attribute_values")
      .insert({ attribute_id, value, color_hex: color_hex || null });
    if (error) {
      toast({ title: "Erreur", description: error.message, variant: "destructive" });
      return false;
    }
...
export function useProductFiles(productId: string | null) {
  ...
  const addFile = async (productId: string, file: File) => {
    ...
    const { data: { publicUrl } } = supabase.storage.from("company-assets").getPublicUrl(path);
    const { error } = await (supabase as any).from("product_files").insert({
      product_id: productId,
      file_name: file.name,
      file_url: publicUrl,
      file_type: file.type,
  ...
  const deleteFile = async (id: string) => {
    const { error } = await (supabase as any).from("product_files").delete().eq("id", id);
    if (error) {
      toast({ title: "Erreur", description: error.message, variant: "destructive" });
      return false;
    }
